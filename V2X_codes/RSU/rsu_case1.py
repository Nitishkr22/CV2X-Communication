...
DEBUG_LOG_PATH    =  "/home/guest/app_logs/"        ### File : case1-applog__rsu-*
DATA_LOG_PATH     =  "/home/guest/data_logs/"       ### File : case1-log__rsu-*
...
def read_case1_log():
   ...
   if (latest_log_file != prev_log_file) and (prev_log_prsnt == True):
      ...
      with open(prev_log_file,"r",os.O_NONBLOCK) as read_file:
         v2i_in = islice(read_file,read_index,read_index+NUM_PKTS_FROM_PREVLOG)
         for line in v2i_in:
            if line != "":
               i2s_out += line
   ...
   ### Normal case : Read from the latest log file
   else:
      prev_log_prsnt = True
      if EN_TRMNL_PRINT:
   ...
   def dsrc_recv(threadName):
      ...
      while  True:
         ...
         if ( msg[0:3]== 'V2I'): # needs to be tested
            ...
            rxd_pkt = "I2STIH," + RSU_ID + "," + msg[4:6] + msg[9:] 
            log_rxdpkt = "I2STIH," + RSU_ID + "," + msg[4:7] + LOG_PKT_TYPE + msg[12:]           
            print ("Rx at rsu:",rxd_pkt)
      ...
      while True:                            
         ...
         ### Case-3: Time to update hbt pkt
            ...
            if (sckt_state == CLOSED):
               ...
               i2s_pkt  = "I2S," + RSU_ID + "," + SOURCE_TYPE + "," + ALIVE_PKT_TYPE + ",00,"+str(hbt_seqno)+"," + CHK_SUM
